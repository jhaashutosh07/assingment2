...
def check_3_config_nonempty_at_init() -> Tuple[bool, str]:
    """Init container logs show non-empty content for BOTH queues.conf and exchanges.conf."""
    ...
    if not re.search(r'queues\.conf:\s+\S', logs):
        return False, "queues.conf empty at init"
    # Match 'exchanges.conf:' followed by any non-whitespace content
    if not re.search(r'exchanges\.conf:\s+\S', logs):
        return False, "exchanges.conf empty at init"
